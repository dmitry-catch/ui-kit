...
import { callSelectOnElement, handleYearInputEvent, numberOfDaysInMonth, handleInitialDateValue } from './utils.js'
...
const handleMonthInput = (event: Event) => {
	const target = event.target as HTMLInputElement
	const newInternalValue = internalValue.value instanceof Date ? internalValue.value : new Date()
	newInternalValue.setMonth(Number(target.value))
	internalValue.value = newInternalValue
}
const handleYearInput = (event: Event) => {
	const target = event.target as HTMLInputElement
	const newInternalValue = internalValue.value instanceof Date ? internalValue.value : new Date()
	newInternalValue.setFullYear(Number(target.value))
	internalValue.value = newInternalValue
}
...
watch(internalValue, () => {
	...
	console.log(internalValue.value?.getFullYear(), internalValue.value?.getMonth(), internalValue.value?.getDate())
	day.value = internalValue.value?.getDate()
	month.value = internalValue.value ? internalValue.value?.getMonth() + 1 : undefined
